...
std::unique_ptr<orb_slam_2_interface::OrbSlam2Interface> create_interface(
    ...
    if (interface_type == "mono") {
        ...
        interface = std::unique_ptr<orb_slam_2_interface::OrbSlam2Interface>(
            new orb_slam_2_interface::OrbSlam2InterfaceStereo(nh, nh_private, use_pangolin));
    } else if (interface_type == "rgbd") {
        interface = std::unique_ptr<orb_slam_2_interface::OrbSlam2Interface>(
            new orb_slam_2_interface::OrbSlam2InterfaceRgbd(nh, nh_private, use_pangolin));
    }else {
        ROS_FATAL(
            "interface type not recognized. Must be mono, stereo or rgbd.");
        ros::shutdown();
        exit(1);
    }
...
int main(int argc, char** argv) {
    ...
    nh_private.param("use_pangolin", use_pangolin, true);
    // Creating the interface object to do the work
    std::unique_ptr<orb_slam_2_interface::OrbSlam2Interface> interface =
        create_interface(interface_type, nh, nh_private, use_pangolin);
    // Spinning
    ros::spin();
    // Exit tranquilly
    return 0;
